...
class BUKUTAMU_Core extends CI_Controller 
{
	/**
	...
	*/
	public function __construct()
	{
		...
		$this->app = load_cache('bukutamu_app', 'app_m', 'getAppSetting', NULL, 300);

		if(empty($this->app)) redirect('konfigurasi-aplikasi');
		...
		if(!empty($this->session->userdata('uid')) && !empty($this->session->userdata('gid'))) 
		{
			...
			$this->user 		= load_cache('bukutamu_user_'.$this->user_hash, 'app_m', 'getUserProfile', NULL, 300);
			
			// Save Loaded App Main Menu to cache
			$this->menus 		= load_cache('bukutamu_menu_'.$this->user_hash, 'app_m', 'getMainMenu', NULL, 300);
		...
		if(config_item('security_headers'))
		{
